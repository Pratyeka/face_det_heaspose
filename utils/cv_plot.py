...
def plot_kpt(image, kpt):
    ...
    for i in range(kpt.shape[0]):
        ...
        # image = cv2.putText(image, str(i), (st[0], st[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
        if i in end_list:
            continue
        ed = kpt[i + 1, :2]
...
def plot_pose_box(image, P, kpt, color=(0, 255, 0), line_width=1):

...
if __name__ == '__main__':
    ...
    print('--->trans matrix:\n', trans)
    print('--->trans_inv matrix:\n', trans_inv)

    print('---> apply transform to uv')
    print('xy_m = uv_augmented * trans')
    uv_aug = np.hstack((
        uv, np.ones((uv.shape[0], 1))
    ))
    ...
    print('xy_m = tformfwd(trans, uv)')
    xy_m = tformfwd(trans, uv)
    print(xy_m)

    print('---> apply inverse transform to xy')
    print('uv_m = xy_augmented * trans_inv')
    xy_aug = np.hstack((
    ...
    print('uv_m = tformfwd(trans_inv, xy)')
    uv_m = tformfwd(trans_inv, xy)
    print(uv_m)

    uv_m = tforminv(trans, xy)
    print('uv_m = tforminv(trans, xy)', uv_m)
